...
const Header = () => {
  ...
  const navItems = [
    ...
    { name: 'Start Now', path: '/start-now', highlight: false }
  ];
      ...
      <div className="container-custom py-4 md:py-5">
        <div className="flex items-center justify-between">
          <Link 
            ...
            aria-label="Home"
          >
            <span className="font-extrabold tracking-tight text-accessible-dark">
              AccessPath
            </span>
            <Sparkles className="inline-block ml-2 text-accessible-dark" size={24} />
          </Link>
            ...
            <ul className="flex flex-col space-y-4">
              {navItems.map((item) => (
                <li key={item.name}>
                  <Link
                    to={item.path}
                    className="block transition-colors duration-200 py-2 px-3 rounded-lg hover:bg-accessible-light/50 text-accessible-text"
                  >
                    {item.name}
                  </Link>
                </li>
